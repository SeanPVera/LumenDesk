...
struct ScenesView: View {
    ...
    var body: some View {
        ...
        .alert("Allow Music-Reactive Lighting?", isPresented: Binding(get: { pendingAudioEffect != nil }, set: { if !$0 { pendingAudioEffect = nil } })) {
            ...
            Text("Music Mode analyzes system audio on the Mac (via the Screen Recording permission) and the microphone on iPhone and iPad. Audio is processed locally, never recorded or retained, and the active effect remains visible with a one-click Stop control.")
        }
