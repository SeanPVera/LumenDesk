...
function Onboarding({ onFinish, onDemo }: { onFinish: () => void; onDemo: () => void }) {
  ...
  return <main className="onboarding-shell">
    <aside className="setup-progress" aria-label="Setup progress">
      <div className="brand-lockup"><span className="brand-mark" /><span>LumenDesk</span></div>
    ...
    <section className="setup-content">
      <div className="setup-card">
        {step === 0 && <>
          <span className="hero-orb" aria-hidden="true"><span className="brand-mark" /></span><p className="eyebrow">The local lighting instrument</p><h1>One desk for every<br /><span className="gradient-text">local light.</span></h1>
          <p className="lede">Control supported LIFX and Govee lights directly on this network, with no account, bridge, or cloud round trip.</p>
          <div className="value-grid"><article><b>Direct control</b><span>Power, level, and color travel straight to the light.</span></article><article><b>Mixed lights, one desk</b><span>Build scenes and rooms across brands.</span></article><article><b>Device truth</b><span>See when a command is sent, applied, confirmed, or needs help.</span></article></div>
        </>}
        {step === 1 && <>
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar"><div className="brand-lockup"><span className="brand-mark" /><span>LumenDesk</span></div><nav aria-label="Primary navigation">{navItems.map((item) => <button key={item.id} className={route === item.id ? 'active' : ''} onClick={() => navigate(item.id)}><span aria-hidden="true">{item.icon}</span>{item.label}{item.id === 'devices' && devices.some((device) => device.connectivity === 'offline') && <i className="nav-alert" aria-label="Device attention needed" />}</button>)}</nav><div className="sidebar-bottom"><div className="local-chip"><span>⌁</span><div><b>Local link</b><small>{devices.filter((d) => d.connectivity === 'online').length} devices online</small></div></div><button className="profile-button" onClick={() => navigate('settings')}><span>SV</span><span><b>Private workspace</b><small>No account required</small></span></button></div></aside>
    <div className="workspace"><header className="topbar"><div><button className="mobile-brand" onClick={() => navigate('home')} aria-label="LumenDesk Home"><span className="brand-mark small" /></button><div><p className="eyebrow">Lighting desk</p><b>{pageTitle}</b></div></div><div className="topbar-actions"><StatusBadge phase={devices.some((item) => item.connectivity === 'offline') ? 'offline' : 'online'} label={`${devices.filter((item) => item.connectivity === 'online').length}/${devices.length} online`} /><button className="button secondary small" onClick={() => { setAnnouncement('Scanning for local lights'); window.setTimeout(() => setAnnouncement('Scan complete. Eight known lights.'), 900) }}>↻ Scan</button><button className="icon-button menubar-button" aria-label="Open menu-bar controller simulation" onClick={() => setMenuBarOpen(true)}><span className="brand-mark small" /></button></div></header>
