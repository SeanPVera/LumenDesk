...
// MARK: - The channel strip
//
// The object the whole system is built on. A strip is a fixture's address, its
// meter, its level, its name, its fader, and its power key, in one column that
// is also lit by the fixture it controls.
//
// It replaces six separate renderings of device state that used to share the
// Home screen: a sidebar summary, a metric tile, a master card, a room card,
// a row's metadata line, and a status badge.

/// What the transport last said about this channel.
enum WashChannelState: Equatable {
    case confirmed
    case inFlight
    case queued
    case failed
    ...
    var dotColour: Color {
        switch self {
        case .confirmed, .inFlight: return Lumen.link
        case .queued: return Lumen.warn
        case .failed: return Lumen.fail
        case .offline: return Lumen.faint
        }
    }
...
/// A vertical fader. The primary control on every strip, and live: dragging it
/// sends the command. Nothing on a strip opens a sheet to change a level.
struct WashVerticalFader: View {
    let label: String
    @Binding var value: Double
    /// The travel fills in the fixture's own colour.
    var fill: Color = Lumen.chalk
    var height: CGFloat = Lumen.faderTravel
    var onEditingChanged: (Bool) -> Void = { _ in }

    @Environment(\.isEnabled) private var isEnabled
    @State private var editing = false

    private var fraction: Double { min(1, max(0, value)) }

    var body: some View {
        GeometryReader { proxy in
            let travel = max(1, proxy.size.height)
            ZStack(alignment: .bottom) {
                ...
                Capsule()
                    .fill(fill)
                    .frame(width: 5, height: travel * fraction)
            }
            .frame(maxWidth: .infinity)
            .overlay(alignment: .bottom) {
                RoundedRectangle(cornerRadius: 3, style: .continuous)
                    ...
                    .shadow(color: Lumen.chalk.opacity(editing ? 0.45 : 0.2), radius: 7)
                    .offset(y: -(travel * fraction) + 6.5)
            }
            .contentShape(Rectangle())
            .gesture(
                DragGesture(minimumDistance: 0)
                    .onChanged { drag in
                        ...
                        commit(y: drag.location.y, travel: travel)
                    }
                    .onEnded { drag in
                        commit(y: drag.location.y, travel: travel)
                        editing = false
        ...
        .focusableCompat()
        .onVerticalMoveCompat { nudge($0) }
    }

    private func commit(y: CGFloat, travel: CGFloat) {
        value = min(1, max(0, Double((travel - y) / travel)))
    }

    private func nudge(_ direction: Int) {
        value = min(1, max(0, value + Double(direction) * 0.05))
        onEditingChanged(false)
    }
}
...
///
/// Deliberately knows nothing about `LightManager`: it takes the values it
/// draws and the bindings it writes, so the same object serves a fixture, a
/// room group, and the master.
struct WashChannelStrip: View {
    /// Channel address, e.g. "04". The master uses "Master".
    let channel: String
    let name: String
    /// The instrument line: vendor, colour temperature, model.
    let meta: String
    /// Three-letter vendor tag. Vendor identity is a tag in Wash, never a hue.
    var vendorTag: String? = nil
    /// The fixture's own colour. Everything coloured on this strip comes from
    /// here, so a field of strips reads the way the room reads.
    let colour: Color
    @Binding var level: Double
    @Binding var isOn: Bool
    var state: WashChannelState = .confirmed
    var isSelected: Bool = false
    var isMaster: Bool = false
    var onSelect: () -> Void = {}
    var onLevelEditing: (Bool) -> Void = { _ in }

    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @State private var pulse = false

    private var width: CGFloat { isMaster ? Lumen.masterStripWidth : Lumen.stripWidth }
    private var shownLevel: Double { isOn ? level : 0 }
    private var travelColour: Color { isMaster ? Lumen.chalk : colour }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            readout
            identity
            WashVerticalFader(label: "Level for \(name)",
                              value: $level,
                              fill: travelColour,
                              onEditingChanged: onLevelEditing)
                .disabled(!state.isReachable)
                .padding(.top, 2)
            Toggle("", isOn: $isOn)
                .labelsHidden()
                .toggleStyle(WashStripPowerKeyStyle(spokenLabel: "Power for \(name)"))
                .disabled(!state.isReachable)
        }
        .padding(9)
        .frame(width: width)
        .background(
            RoundedRectangle(cornerRadius: Lumen.stripRadius, style: .continuous)
                .fill(isSelected || isMaster ? Lumen.stripRaised : Lumen.strip)
        )
        .washed(color: colour, level: level, isOn: isOn && state.isReachable && !isMaster)
        .overlay {
            if isSelected {
                RoundedRectangle(cornerRadius: Lumen.stripRadius, style: .continuous)
                    .stroke(Lumen.rule, lineWidth: 1)
            }
        }
        .overlay {
            if !state.isReachable {
                RoundedRectangle(cornerRadius: Lumen.stripRadius, style: .continuous)
                    .fill(Lumen.stage.opacity(0.35))
        ...
        .accessibilityValue("\(Int((shownLevel * 100).rounded())) percent, \(state.spokenName)")
    }
    ...
    private var header: some View {
        HStack(spacing: 6) {
            Text(channel)
                ...
                .foregroundStyle(isMaster ? Lumen.meter : Lumen.faint)
            if !isMaster {
                Circle()
                    .fill(state.dotColour)
                    .frame(width: 5, height: 5)
                    .opacity(state == .inFlight && pulse ? 0.25 : 1)
                    .shadow(color: state.dotColour.opacity(state == .confirmed ? 0.6 : 0), radius: 4)
            }
            Spacer(minLength: 0)
            if let vendorTag {
                Text(vendorTag)
                    .font(LumenType.readout(size: 8.5, weight: .medium))
                    .foregroundStyle(Lumen.faint)
            }
        }
        .frame(height: 14)
        .onAppear {
            guard state == .inFlight, !reduceMotion else { return }
    ...
    /// The meter and the level share one band, so the numbers across a field of
    /// strips land on a single baseline and can be compared by scanning it.
    private var readout: some View {
        HStack(alignment: .bottom, spacing: 8) {
            ...
            HStack(alignment: .firstTextBaseline, spacing: 1) {
                Text(state.isReachable ? "\(Int((shownLevel * 100).rounded()))" : "––")
                    .font(LumenType.readout(size: 25, weight: .medium))
                    .monospacedDigit()
                    .foregroundStyle(state.isReachable ? Lumen.chalk : Lumen.faint)
                Text("%")
                    .font(LumenType.readout(size: 10, weight: .medium))
                    .foregroundStyle(Lumen.muted)
            }
            // The readout stays legible over a 100% white fixture washing the
            // strip underneath it.
            .shadow(color: Lumen.stage.opacity(0.9), radius: 5)
        }
        .frame(height: 46)
    }
    ...
    private var identity: some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            Text(meta)
                .font(LumenType.readout(size: 9, weight: .regular))
                .foregroundStyle(state.isReachable ? Lumen.muted : Lumen.faint)
                .lineLimit(1)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
    }
...
/// The power key as it appears on a strip: full width, and lit when the
/// fixture is on, so a field of powered strips reads from across the room.
struct WashStripPowerKeyStyle: ToggleStyle {
    var spokenLabel: String? = nil

    func makeBody(configuration: Configuration) -> some View {
        WashStripPowerKeyFace(configuration: configuration, spokenLabel: spokenLabel)
    }
}

private struct WashStripPowerKeyFace: View {
    let configuration: ToggleStyleConfiguration
    let spokenLabel: String?
    ...
    var body: some View {
        Button {
            ...
            ZStack {
                shape.fill(configuration.isOn ? Lumen.lit : Lumen.stage)
                Image(systemName: "power")
            ...
            .frame(maxWidth: .infinity)
            .frame(height: 30)
            .shadow(color: Lumen.chalk.opacity(configuration.isOn ? 0.3 : 0), radius: 12)
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
        .opacity(isEnabled ? 1 : 0.25)
