...
function RoomCard({ room, devices, onOpen, onPower }: { room: Room; devices: Device[]; onOpen: () => void; onPower: (value: boolean) => void }) {
  ...
  return <article className="room-card interactive-card" tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter') onOpen() }}>
...
function HomeView({ rooms, devices, commands, scenes, activeEffect, demoMode, onOpenRoom, onOpenLight, onNavigate, runCommand, retryCommand, setDevices }: {
  ...
  return <>
    ...
    <section className="section-block lights-section">
      <div className="section-heading responsive"><div><p className="eyebrow">Direct control</p><h2>Individual lights</h2></div><div className="toolbar-actions"><button className={cx('button small', selecting ? 'primary' : 'secondary')} onClick={() => { setSelecting(!selecting); if (selecting) setSelected([]) }}>{selecting ? 'Done' : 'Select'}</button><button className="segmented-button" aria-label="Interface density"><span className={density === 'comfortable' ? 'active' : ''} onClick={() => setDensity('comfortable')}>▦</span><span className={density === 'compact' ? 'active' : ''} onClick={() => setDensity('compact')}>☷</span></button></div></div>
